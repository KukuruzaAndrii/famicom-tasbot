...
#define DMA1_ENABLE BIT(21)

/* On APB1 */
#define UART2_ENABLE 0x20000
#define TIM2_ENABLE BIT(0)
 ...
 */
static void cpu_clock_init_pll(void) {
 ...
 * 96 Mhz
 */
static void cpu_clock_init_hsi(void) {
	struct rcc *rp = RCC_BASE;
	unsigned int xyz;
...
static void cpu_clock_init_32(void) {
	struct rcc *rp = RCC_BASE;
	unsigned int xyz;
 ...
 */
static void cpu_clock_init_25(void) {
	struct rcc *rp = RCC_BASE;
	unsigned int xyz;
...
void rcc_init(void) {
	...
	rp->ahb1_e |= DMA1_ENABLE;

	rp->apb1_e |= UART2_ENABLE;
	rp->apb1_e |= TIM2_ENABLE;
